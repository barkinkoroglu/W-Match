    ...
    <div className="relative group/edit  bg-white shadow-md rounded-lg mb-4 mt-5">
      <div className="relative p-4">
        <div className="flex justify-between items-center">
          <h1 className="text-lg font-medium mb-2 flex items-center gap-2">
            <AiOutlineAppstoreAdd className="text-blue-500 mt-1" />
            {data.jobname}
          </h1>
          <a href={`profile/${company?.username}`}>
        ...
        {user?.type === 1 && (
          <button
            onClick={() => handleApply()}
            className="bg-blue-500 px-2 py-1 rounded-lg absolute right-4 bottom-4 text-white font-semibold hover:bg-blue-600 transition-colors duration-200"
          >
            Apply
          </button>
        )}
